...
class Chatbot:
    ...
    def refresh_headers(self) -> None:
        if "Authorization" not in self.config or not self.config["Authorization"]:
    ...
    def get_chat_text(self, data) -> dict:
        ...
        s.cookies.set("__Secure-next-auth.callback-url", "https://chat.openai.com/")
        # Set proxies
        ...
        try:
            ...
            try:
                ...
                if error_desp.get("message"):
                    error_desp = error_desp["message"]
            finally:
                print(response.text)
                raise ValueError("Response is not in the correct format", error_desp) from exc
    ...
    def login(self, email, password) -> None:
        ...
        if auth.access_token is not None:
            ...
            if auth.session_token is not None:
                ...
                possible_tokens = auth.session.cookies.get("__Secure-next-auth.session-token")
...
class OpenAIAuth:
    ...
    def part_eight(self, old_state: str, new_state) -> None:
        ...
        if is_200:
            # Access Token
            access_token = re.findall(r"accessToken\":\"(.*)\"", response.text)
            if access_token:
